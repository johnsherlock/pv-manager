...
| U-023 | UX / Visual Design | Build first polished clickable prototype for the Live screen | Turn the Live screen into the first polished clickable prototype to validate layout, hierarchy, and trust treatment before production implementation. | A polished static prototype exists for Live on desktop and mobile at `/prototype/live`, including healthy, stale, and disconnected states, with clear trust/freshness treatment. Also establishes Tailwind CSS, shadcn/ui, Recharts, and lucide-react in `apps/web`. | U-004 | Done | Prototype at `/prototype/live?state=healthy\|stale\|warning`; use `?state=` param to switch states. |
