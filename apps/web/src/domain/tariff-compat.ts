...
// Stable synthetic IDs used when constructing periods from window fields.
// Using deterministic values (not random UUIDs) makes the output predictable
// for tests and migration scripts.
const SYNTHETIC_DAY_ID   = 'compat-day';
const SYNTHETIC_NIGHT_ID = 'compat-night';
const SYNTHETIC_PEAK_ID  = 'compat-peak';
...
export function migrateWindowsToSchedule(tariff: TariffVersion): {
  ...
  for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
    for (let slotIndex = 0; slotIndex < 48; slotIndex++) {
      // Format the slot start time as "HH:MM"
      ...
      if (
        peakPeriod &&
        inWindow(slotTime, tariff.peakStartLocalTime, tariff.peakEndLocalTime)
      ) {
        schedule.push(SYNTHETIC_PEAK_ID);
      } else if (
        nightPeriod &&
        inWindow(slotTime, tariff.nightStartLocalTime, tariff.nightEndLocalTime)
      ) {
        schedule.push(SYNTHETIC_NIGHT_ID);
      } else {
        schedule.push(SYNTHETIC_DAY_ID);
      }
    }
  }
